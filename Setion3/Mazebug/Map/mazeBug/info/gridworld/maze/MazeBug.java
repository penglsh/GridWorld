...
public class MazeBug extends Bug {
	...
	private void moveBack() {
		...
		if (crossLocation.size() > 0) {
			...
			if (crossLocation.size() > 0) {
				...
				deleteDirTime(dir, gr, getLocation());
			}
		}
	}
	...
	public ArrayList<Location> getValid(Location loc) {
		...
		for (int i = 0; i < 4; i ++) {
			...
			if (gr.isValid(neigh)) {
				...
				else if (testAc == null || testAc instanceof Flower) {
					valid.add(neigh);
				}
			}
	...
	/*
	 * this method is to judge whether the bug reaches the destination, 
	 * that is, the red rock.
	 */
	private void judgeReachDestination() {
